...
const levelColors: Record<string, string> = {
  BEGINNER: '#8899aa', INTERMEDIATE: '#f39c12', EXPERT: '#0a66c2',
};

const projectColors: Record<string, { bg: string; label: string }> = {
...
type ConnectionStatus = 'ACCEPTED' | 'PENDING_SENT' | 'PENDING_INCOMING' | null;

function PortfolioCard({ item, isOwn, onDelete }: { item: PortfolioItem; isOwn: boolean; onDelete: (id: number) => void }) {
...
function ProfilePostCard({ post, idx, postsLen, myUsername, token }: {
  post: Post; idx: number; postsLen: number; myUsername: string | null; token: string | null;
}) {
  ...
  return (
    <div style={{
      ...
      borderLeft:   post.isProject ? `4px solid ${projectColor}` : 'none',
    }}>
      {post.isProject && (
...
function Profile() {
  ...
  usePageMeta(
    user ? `${user.fullName}'s Profile` : 'Profile',
    user ? `${user.fullName} — ${user.roles?.[0]?.replace('_', ' ')} on TheFilmProject` : undefined,
  );
    ...
    try {
      ...
      if (token && !isOwn) {
        try {
          ...
          const conn = connRes.data.find((c: any) => c.sender.id === userRes.data.id || c.receiver.id === userRes.data.id);
          if (conn) { setConnectionStatus(conn.status); setConnectionId(conn.id); return; }
          const pendingRes = await api.get('/connections/pending');
          const pending = pendingRes.data.find((c: any) => c.sender.id === userRes.data.id);
          ...
          const sent = sentRes.data.find((c: any) => c.receiver.id === userRes.data.id);
  ...
  const respondConnection = async (accept: boolean) => {
    try {
      await api.patch(`/connections/${connectionId}/respond?accept=${accept}`);
      setConnectionStatus(accept ? 'ACCEPTED' : null);
    } catch {}
  };

  const deletePortfolioItem = async (id: number) => {
    try { await api.delete(`/portfolio/${id}`); setPortfolio(prev => prev.filter(p => p.id !== id)); } catch {}
  };
  ...
  if (loading) return <div className="profile__loading"><p>Loading...</p></div>;
  if (!user)   return <div className="profile__loading"><p>User not found</p></div>;
            ...
            <div className="profile__card-body">
              <div className="profile__header-row">
                <div className="profile__avatar-wrapper">
                  <Avatar photoUrl={user.profilePhotoUrl} name={user.fullName} size={100} editable={isOwn} onUpdated={handlePhotoUpdated} />
                </div>
                <div className="profile__actions">
                  {isOwn ? (
                    ...
                    {connectionStatus === 'ACCEPTED' && (
                      <button onClick={() => navigate(`/messages?user=${user.id}&name=${user.fullName}`)} className="profile__btn profile__btn--message">💬 Message</button>
                    )}
                  </>)}
                </div>
              </div>
              ...
              <div className="profile__roles">
                {user.roles?.map(role => (
                  <span key={role} className="profile__role-badge" style={{
                    background: `${roleColors[role as UserRole] || '#0a66c2'}18`,
                    color: roleColors[role as UserRole] || '#0a66c2',
                    border: `1px solid ${roleColors[role as UserRole] || '#0a66c2'}44`,
                  }}>
                    {role.replace('_', ' ')}
                  </span>
                ))}
            ...
            <div className="profile__card">
              ...
              {showAddPortfolio && (
                <div className="profile__portfolio-form">
                  <input placeholder="Project title *" value={newItem.title}
                  ...
                  <input placeholder="YouTube URL (optional)" value={newItem.videoUrl}
                    onChange={e => setNewItem({ ...newItem, videoUrl: e.target.value })} />
                  <input placeholder="Image URL (optional)" value={newItem.imageUrl}
